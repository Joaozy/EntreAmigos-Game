const { generateDeck } = require('../utils/helpers');
const THEMES_ITO = require('../data/themes.json'); // Certifique-se que este arquivo existe

const startIto = (io, room, roomId) => {
    const deck = generateDeck(); // Gera [1...100]
    
    // Escolhe tema aleatório
    const theme = THEMES_ITO && THEMES_ITO.length > 0 
        ? THEMES_ITO[Math.floor(Math.random() * THEMES_ITO.length)] 
        : { title: "Tema Livre", min: "Pequeno", max: "Grande" };

    room.gameData = { theme }; 
    room.phase = 'GAME'; 
    
    room.players.forEach(p => { 
        p.secretNumber = deck.pop(); 
        p.clue = ''; 
        p.hasSubmitted = false;
        delete p.isCorrect; // Limpa estado anterior
    });
    
    io.to(roomId).emit('game_started', { 
        gameType: 'ITO', 
        phase: 'CLUE_PHASE', 
        gameData: room.gameData, 
        players: room.players 
    });
    
    // Envia número secreto individualmente
    room.players.forEach(p => io.to(p.id).emit('your_secret_number', p.secretNumber));
};

const registerItoHandlers = (io, socket, rooms) => {
    // 1. Receber Dica
    socket.on('submit_clue', ({ roomId, clue }) => {
        const room = rooms.get(roomId); if(!room) return;
        const p = room.players.find(x => x.id === socket.id);
        
        if(p) { 
            p.clue = clue; 
            p.hasSubmitted = true;
            
            // Se todos enviaram, avança fase
            if(room.players.every(x => x.hasSubmitted)) {
                io.to(roomId).emit('phase_change', { phase: 'ORDERING', players: room.players });
            } else {
                io.to(roomId).emit('player_submitted', { playerId: socket.id });
            }
        }
    });

    // 2. Atualizar Ordem (Drag and Drop)
    socket.on('update_order', ({ roomId, newOrderIds }) => {
        const room = rooms.get(roomId); if(!room) return;
        
        const reordered = []; 
        newOrderIds.forEach(id => { 
            const p = room.players.find(pl => pl.id === id); 
            if(p) reordered.push(p); 
        });
        
        room.players = reordered; 
        socket.to(roomId).emit('order_updated', room.players);
    });

    // 3. Revelar Cartas
    socket.on('reveal_cards', ({ roomId }) => {
        const room = rooms.get(roomId); if(!room || room.host !== socket.id) return; 
        
        room.phase = 'REVEAL';
        
        // Ordena para saber qual seria o correto
        const perfectOrder = [...room.players].sort((a, b) => a.secretNumber - b.secretNumber);
        
        let totalScore = 0;
        const results = room.players.map((player, index) => {
            const isCorrect = player.id === perfectOrder[index].id; 
            if (isCorrect) totalScore++;
            return { ...player, isCorrect, secretNumber: player.secretNumber };
        });
        
        io.to(roomId).emit('game_over', { results, totalScore, maxScore: room.players.length });
    });

    // 4. Reinício Rápido (Corrigido)
    socket.on('ito_restart', ({ roomId }) => {
        const room = rooms.get(roomId);
        if (room && room.host === socket.id) {
            console.log(`[ITO] Reinício imediato na sala ${roomId}`);
            startIto(io, room, roomId);
        }
    });
};

// --- CORREÇÃO DE RECONEXÃO ---
const handleRejoin = (io, socket, room, oldId, newId) => {
    const me = room.players.find(p => p.id === newId);
    
    // Reenvia o número secreto para o jogador que voltou
    if (me && me.secretNumber !== undefined) {
        socket.emit('your_secret_number', me.secretNumber);
    }
    
    // Atualiza tracking de submissão
    if (room.gameData && room.gameData.answers && room.gameData.answers[oldId]) {
         room.gameData.answers[newId] = room.gameData.answers[oldId];
         delete room.gameData.answers[oldId];
    }

    return false; // ITO não precisa de update global de gameData no rejoin
};

module.exports = { startIto, registerItoHandlers, handleRejoin };