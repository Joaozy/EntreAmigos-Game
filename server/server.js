...
io.on('connection', (socket) => {
    ...
    if (player) {
      ...
      if (allReady) {
        ...
        io.to(roomId).emit('phase_change', { 
          phase: 'ORDERING',
          players: room.players.map(p => ({ id: p.id, nickname: p.nickname, clue: p.clue }))
        });
      } else {
  ...
  socket.on('update_order', ({ roomId, newOrderIds }) => {
    ...
    socket.to(roomId).emit('order_updated', room.players.map(p => ({
        id: p.id, nickname: p.nickname, clue: p.clue
    })));
  });
...
function joinRoomInternal(socket, roomId, nickname, isHost) {
  ...
  socket.emit('joined_room', { 
    roomId, isHost, players: room.players, phase: room.phase, theme: room.currentTheme 
  });
}
