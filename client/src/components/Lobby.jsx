...
const GAMES = [
    ...
    { id: 'MEGAQUIZ', name: 'MegaQuiz', icon: '🧠', color: 'bg-purple-600' },
    { id: 'WHOAMI', name: 'Quem Sou Eu', icon: '🤔', color: 'bg-yellow-500' },
];
    ...
    <div className="min-h-screen bg-slate-900 p-6 flex flex-col items-center">
      ...
      {mode === 'MENU' && (
          <div className="w-full max-w-md grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-4">
              <button onClick={() => setMode('CREATE')} className="col-span-2 bg-gradient-to-r from-indigo-600 to-purple-600 p-8 rounded-3xl text-white font-black text-2xl shadow-xl hover:shadow-2xl hover:scale-[1.02] transition transform">
                  CRIAR SALA 🏠
              </button>
              <button onClick={() => setMode('JOIN')} className="col-span-2 bg-slate-700 p-6 rounded-2xl text-white font-bold text-lg shadow-lg hover:bg-slate-600 transition border-2 border-slate-600">
                  ENTRAR EM SALA 🔑
              </button>
          </div>
      )}
      ...
      {mode === 'CREATE' && (
          <div className="w-full max-w-md animate-in fade-in slide-in-from-right">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6 max-h-[60vh] overflow-y-auto pr-2">
                  {GAMES.map(g => (
                      <button 
                        key={g.id}
                        onClick={() => setSelectedGame(g.id)}
                        className={`p-4 rounded-xl text-left transition border-2 relative overflow-hidden ${selectedGame === g.id ? 'border-white bg-white/10' : 'border-transparent bg-slate-800 hover:bg-slate-700'}`}
                      >
                          ...
                          <span className={`text-sm font-bold ${selectedGame === g.id ? 'text-white' : 'text-slate-400'}`}>{g.name}</span>
                      </button>
                  ))}
              </div>
              <button onClick={criarSala} className="w-full bg-green-500 py-4 rounded-xl font-black text-white shadow-lg hover:bg-green-400 transition transform active:scale-95 text-lg">
                  INICIAR JOGO
              </button>
