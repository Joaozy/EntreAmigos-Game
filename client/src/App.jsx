...
import GameCodenames from './GameCodenames'; // Código Secreto
import GameStop from './GameStop';           // Stop!
import Chat from './Chat';

// Ícones
import { Trash2, ArrowRight, Gamepad2, Info, Coffee, Loader2, LogOut, Eye, Hand } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on('game_over', (data) => {
      if(data.winnerWord || data.winner) { 
  ...
  if (view === 'LOADING') return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-4 text-center">
          ...
          <button onClick={sairDoJogo} className="mt-8 text-red-400 text-sm border border-red-900/50 p-2 rounded bg-red-900/20">Cancelar</button>
      </div>
  );
  ...
  if (view === 'HOME') return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-7xl">
        {/* ITO */}
        <div onClick={() => { setSelectedGame('ITO'); setView('LOGIN'); }} className="group bg-slate-800 rounded-3xl p-6 cursor-pointer border-2 border-slate-700 hover:border-indigo-500 transition hover:-translate-y-2">
            <div className="w-16 h-16 bg-indigo-500 rounded-2xl flex items-center justify-center mb-4 text-white shadow-lg"><span className="text-3xl font-black">?</span></div>
            <h2 className="text-xl font-bold text-white">ITO</h2>
            <p className="text-slate-400 text-xs mt-2">Sincronia e cooperação.</p>
        </div>
        {/* CHÁ OU CAFÉ */}
        <div onClick={() => { setSelectedGame('CHA_CAFE'); setView('LOGIN'); }} className="group bg-slate-800 rounded-3xl p-6 cursor-pointer border-2 border-slate-700 hover:border-pink-500 transition hover:-translate-y-2">
            <div className="w-16 h-16 bg-pink-500 rounded-2xl flex items-center justify-center mb-4 text-white shadow-lg"><Coffee size={32} /></div>
            <h2 className="text-xl font-bold text-white">Chá ou Café?</h2>
            <p className="text-slate-400 text-xs mt-2">Adivinhação por contexto.</p>
        </div>
        {/* CÓDIGO SECRETO */}
        <div onClick={() => { setSelectedGame('CODENAMES'); setView('LOGIN'); }} className="group bg-slate-800 rounded-3xl p-6 cursor-pointer border-2 border-slate-700 hover:border-emerald-500 transition hover:-translate-y-2">
            <div className="w-16 h-16 bg-emerald-500 rounded-2xl flex items-center justify-center mb-4 text-white shadow-lg"><Eye size={32} /></div>
            <h2 className="text-xl font-bold text-white">Código Secreto</h2>
            <p className="text-slate-400 text-xs mt-2">Times, espiões e palavras.</p>
        </div>
        {/* STOP */}
        <div onClick={() => { setSelectedGame('STOP'); setView('LOGIN'); }} className="group bg-slate-800 rounded-3xl p-6 cursor-pointer border-2 border-slate-700 hover:border-purple-500 transition hover:-translate-y-2">
            <div className="w-16 h-16 bg-purple-500 rounded-2xl flex items-center justify-center mb-4 text-white shadow-lg"><Hand size={32} /></div>
            <h2 className="text-xl font-bold text-white">Stop!</h2>
            <p className="text-slate-400 text-xs mt-2">Adedonha clássica rápida.</p>
        </div>
      </div>
    </div>
  );
      ...
      return (
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-white rounded-3xl shadow-2xl p-8 space-y-6 relative animate-in zoom-in-95">
            ...
            {/* BOTÃO CORRIGIDO: Classes explícitas */}
            <button 
                className={`w-full text-white p-4 rounded-xl font-bold shadow-lg transition ${theme.btnClass}`} 
                onClick={criar} 
            ...
            <div className="flex gap-2">
                <input className="flex-1 bg-slate-100 border-2 border-slate-200 p-4 rounded-xl font-bold text-center outline-none uppercase" placeholder="CÓDIGO" value={roomId} onChange={e => setRoomId(e.target.value)} maxLength={4}/>
                <button className="bg-slate-800 text-white px-6 rounded-xl font-bold" onClick={entrar} disabled={isJoining || !nickname || !roomId}>Entrar</button>
            </div>
          </div>
        </div>
      );
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center pt-10 p-4">
      <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-2xl text-center relative animate-in zoom-in-95">
        <div className="flex justify-between items-start mb-6">
            ...
            <h1 className="text-4xl font-black text-slate-800 cursor-pointer" onClick={() => navigator.clipboard.writeText(roomId)}>{roomId}</h1>
            <div className="w-8"></div>
        </div>
        <div className="text-left mb-8 bg-slate-50 p-6 rounded-2xl">
          <h3 className="font-bold text-slate-500 text-sm uppercase mb-4">Jogadores ({players.length})</h3>
          <ul className="space-y-3">
            {players.map(p => (
              <li key={p.id} className="flex items-center gap-3 group">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white ${p.isHost ? 'bg-slate-800' : 'bg-slate-400'}`}>{p.nickname[0].toUpperCase()}</div>
                <span className="font-bold text-slate-700 flex-1">{p.nickname}</span>
                {isHost && !p.isHost && <button onClick={() => expulsar(p.id)} className="text-red-300 hover:text-red-500"><Trash2 size={16} /></button>}
              </li>
            ))}
          </ul>
        </div>
        {isHost ? (
          <button className="w-full text-white bg-slate-800 p-4 rounded-xl font-bold text-lg shadow-xl hover:scale-105 transition" onClick={iniciar} disabled={players.length < 2}>INICIAR JOGO 🚀</button>
        ) : (
  ...
  return (
    <>
      <div className="fixed top-4 left-4 z-50">
          <button onClick={sairDoJogo} className="bg-slate-800/50 hover:bg-red-600 text-white p-2 rounded-full shadow-lg transition backdrop-blur-sm"><LogOut size={20} /></button>
      </div>
